...
export const api = {
  ...
  requestPasswordReset: (email: string) =>
    request<{ message: string; reset_token: string | null }>("/auth/password-reset/request", null, {
      method: "POST",
  ...
  updateMemberRole: (token: string, workspaceId: number, memberId: number, role: string) =>
    request<unknown>(`/workspaces/${workspaceId}/members/${memberId}`, token, {
      method: "PATCH",
  ...
  invitations: (token: string, workspaceId: number) =>
    request<{ invitations: unknown[]; pending_count: number }>(`/workspaces/${workspaceId}/invitations`, token),
  resendInvitation: (token: string, workspaceId: number, invitationId: number) =>
    request<unknown>(`/workspaces/${workspaceId}/invitations/${invitationId}/resend`, token, { method: "POST" }),
  revokeInvitation: (token: string, workspaceId: number, invitationId: number) =>
    request<unknown>(`/workspaces/${workspaceId}/invitations/${invitationId}/revoke`, token, { method: "POST" }),
  acceptWorkspaceInvitation: (token: string, inviteToken: string) =>
    request<unknown>("/workspaces/invitations/accept", token, {
      method: "POST",
  ...
  createCollection: (token: string, workspaceId: number, name: string) =>
  ...
  upload: (token: string, file: { uri: string; name: string; mimeType?: string }, workspaceId?: number | null) => {
  ...
  documentExportMarkdown: async (token: string, id: number) => {
    const response = await fetch(`${API_URL}/documents/${id}/export?format=markdown`, {
  ...
  createAnnotation: (token: string, id: number, pageNumber: number, note: string, quoteText?: string) =>
    request<DocumentAnnotation>(`/documents/${id}/annotations`, token, {
      method: "POST",
      body: JSON.stringify({ page_number: pageNumber, note, quote_text: quoteText || null, color: "yellow" })
    }),
